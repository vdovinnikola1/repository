...
while True:
    user_сity = input().lower()
    if user_сity == 'сдаюсь': print('Вы проиграли'); break
    if user_сity not in cities: print('Нет такого города')
    elif user_сity in used_cities: print('Город уже называли')
    elif user_сity[0] != last_letter and last_letter != '': print('Неверный город')
    else:
        used_cities.append(user_сity)
        if user_сity[-1] in 'цыьъ':
            last_letter = user_сity[-2]
        else: 
            last_letter = user_сity[-1]
        for x in cities:
            if x[0] == last_letter and x not in used_cities:
                print(x.capitalize())
                if x[-1] not in 'цыьъ':
                    print(f'Вам город на букву {x[-1]}')
                    last_letter = x[-1]
                else:
                    print(f'Вам город на букву {x[-2]}')
                    last_letter = x[-2]
                used_cities.append(x)
                break
        else:
            print("Вы победили")
            break
